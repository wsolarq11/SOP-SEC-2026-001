...
- **`backup\` 是可重建仓库 bundle 暂存区**：post-commit hook 每次提交后重建并同名覆盖上传到飞书 90 目录（`BACKUP_WIKI` Wiki 节点）；默认 `C:/Users/11058/AppData/Local/Temp/sop-exports/backup`，可用 `BACKUP_DIR` 覆盖
